...
Namespace w
	...
	Public Class mgr
					...
					If flag Then
						...
						If flag Then
							...
							If flag Then
								...
								Dim hwnd As Integer = CInt(list(0))
								Dim wMsg As Integer = 12
								Dim wParam As Integer = 0
								Dim text3 As String = "End process"
								mgr.SendMessage(hwnd, wMsg, wParam, text3)
							End If
						End If
					End If
				End If
		...
		Private Function GetChild(hwd As IntPtr) As IntPtr()
			...
			SyncLock Me
				Me.CLD.Clear()
				Dim lpEnumFunc As mgr.EnumWindProc = AddressOf Me.EnumChild
				Dim intPtr As IntPtr = CType(0, IntPtr)
				mgr.EnumChildWindows(hwd, lpEnumFunc, intPtr)
				result = Me.CLD.ToArray()
			End SyncLock
			Return result
		End Function
